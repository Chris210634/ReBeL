...
int main(int argc, char* argv[]) {
  ...
  if (!net_path.empty()) {
    ...
    std::shared_ptr<IValueNet> net =
        net_path == "zero"
            ? poker_dice::create_zero_net(game.num_hands(), false)
            : poker_dice::create_torchscript_net(net_path);
    ...
    if (num_repeats > 0) {
      ...
      auto net_builder = [=]() {
        if (repeat_oracle_net) {
          ...
          return poker_dice::create_oracle_value_predictor(game,
                                                           oracle_net_params);
        } else {
          return poker_dice::create_torchscript_net(net_path, "cpu");
        }
      };
