...
TEST(EvaluateTerninalNode, TestTwoDiceOneHotBelief) {
  ...
  for (int ophand = 0; ophand < game.num_hands(); ++ophand) {
    ...
    for (Action bet = 0; bet < game.num_actions() - 1; ++bet) {
      ...
      for (int myhand = 0; myhand < game.num_hands(); ++myhand) {
        //std::cerr << "myhand=" << myhand << " ophand=" << ophand
        //           << " bet=" << game.action_to_string(bet) << " value: "
        //        << values[myhand] << std::endl;
        int matches = 0;
...
TEST(EvaluateTerninalNode, MyTest) {
  const int num_dice = 2;
  const int num_faces = 3;
  Game game(num_dice, num_faces);
...
int to_score(int a, int b, int c, int d, int e)
{
  return a*36*36+b*36*6+c*36+d*6+e;
}
/*
TEST(PokerScoreTest, MyTest) {
  poker_dice::Game g(6,2);
  //std::cerr << g.utility(5, 5, 6) << std::endl;
  int hands[7];
  for (int it = 0 ;it<100 ; it++)
  {
    for (int i=0;i<7;i++) hands[i] = rand() % 6;
    double u = g.utility(hands);
    std::cerr << "(" << hands[0] << "," << hands[1] << ") (" << 
    hands[4] << "," << hands[5] << "," << hands[6] << ") (" << hands[2] << "," << hands[3];
    std::cerr << ") " << u << std::endl;
  }
  g.print_score(g.score_table[to_score(1, 4, 1, 1, 1)]);
  g.print_score(g.score_table[to_score(2, 1, 3, 4, 5)]);
  g.print_score(g.score_table[to_score(5, 4, 3, 1, 1)]);
  g.print_score(g.score_table[to_score(1, 4, 1, 3, 5)]);
  g.print_score(g.score_table[to_score(1, 4, 3, 1, 5)]);
}
